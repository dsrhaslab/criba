...
function updateIndex {
	...
	while [ $tries -gt 0 ]; do
		...
		response=$(curl -u "$ES_USERNAME:$ES_PASSWORD" -s -X GET "$URL/$INDEX-paths/_count")
	...
	while [ $i -gt 0 ]; do
		...
		if [ $CREAT_POLICY -eq 0 ]; then
			create_enrich_policies $URL $INDEX
			CREAT_POLICY=$?
			...
			execute_enrich_policies $URL $INDEX
			create_ingest_pipeline $URL $INDEX
		else
			execute_enrich_policies $URL $INDEX
		fi
		update_by_query $URL $INDEX $N_TRIES
		i=$?
	...
	delete_enrich_policy $URL $INDEX
	delete_ingest_pipeline $URL $INDEX
}
...
function correlate_data {
	...
	echo "URL: $URL, SLEEP: $SLEEP, N_TRIES: $N_TRIES, $4"
    ...
    while read dir action file; do
		...
		if [[ $action == "CREATE" ]]; then
			...
			INDEX="criba_trace_$SESSION_NAME"
			updateIndex
	...
	# while true; do
	# 	FILE=/tmp/dio/dio_index
	# 	while [ ! -f $FILE ]
	# 	do
	# 		if [[ "$WAITING" = false ]]; then echo $(date)" | File '$FILE' does not exist. Waiting for a index..."; fi
	# 		WAITING=true
	# 		sleep $TIME_WAITING_FOR_INDEX
	# 	done
	# 	# INDEX=`cat $FILE`
	# 	# WAITING=false
	# 	# CREAT_POLICY=0
...
function correlate_index {
	echo "Correlating paths for session $4..."
	URL=$1
	SLEEP=$2
	N_TRIES=$3
	SESSION_NAME=$4
	INDEX="criba_trace_$SESSION_NAME"
	echo "URL: $URL, SLEEP: $SLEEP, N_TRIES: $N_TRIES, INDEX: $INDEX"
	updateIndex
}

# usage:
# correlate_data <elasticsearch_url> <sleep_time> <n_tries> <wait_for_events>
# or
# correlatE_index <elasticsearch_url> <sleep_time> <n_tries> <session_name>
